...
products = []
products_page = requests.get('https://www.homelegance.com/dining/')
...
for index in range(2,23):
    products_page = requests.get('https://www.homelegance.com/dining/page/{0}/'.format(index))
    product_tree = html.fromstring(products_page.content)
    for index in range(1,11):
        if product_tree.xpath('//*[@id="catList"]/li[{0}]/a/@href'.format(index)):
            product_path = product_tree.xpath('//*[@id="catList"]/li[{0}]/a/@href'.format(index))[0]
            product_paths.append(product_path)
...
for path in product_paths:
    ...
    for index in range(2,items_list_length + 1):
        ...
        if len(tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[1]'.format(index))) > 0:
            if tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[1]'.format(index))[0].text != None:
                item_number = tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[1]'.format(index))[0].text.encode("utf-8")
        else:
            item_number = ''
        if len(tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[2]'.format(index))) > 0:
            if tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[2]'.format(index))[0].text != None:
                    item_description = tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[2]'.format(index))[0].text.encode("utf-8")
        else:
            item_description = ''
        if len(tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[3]'.format(index))) > 0:
            if tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[3]'.format(index))[0].text != None:
                item_dimensions = tree.xpath('//*[@id="tabProdInfo"]/table/tr[{0}]/td[3]'.format(index))[0].text.encode("utf-8")
        else:
            item_dimensions = ''
...
with open('dining_items.csv', "wb") as csv_file:
